...
def filter_by_description(data_list: list[dict], str_search: str) -> list[dict]:
    ...
    if len(data_list) == 0:
        logger.error(f"ОШИБКА: Список пустой")
        exit(f'Работа программы завершена')
    elif len(str_search) == 0:
        logger.error(f"ОШИБКА: Поисковый запрос отсутствует")
        exit(f'Работа программы завершена')
    else:
        logger.info(f"Перебор списка словарей")
    ...
    logger.info(f"Завершение программы и вывод результата")
    return new_list
...
def count_description(data_list: list[dict], list_category) -> dict:
    ...
    try:
        if len(data_list) == 0 or len(list_category) == 0:
            logger.error(f"ОШИБКА: Список  или тритерии поиска пустые")
            exit("Даные или критерии не обнаружены.\nРабота программы завершена.")
        logger.info(f"Создаем список из описаний операций")
        for item in data_list:
            list_only_category.append(item["description"])
        logger.info(f"Сравниваем список категорий со списком критерий")
